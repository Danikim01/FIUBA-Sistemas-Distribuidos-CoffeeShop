...
class RabbitMQMiddlewareQueue(MessageMiddleware):
    ...
    Con gestión robusta de conexiones y reconexión automática.
    """
    ...
    def __init__(self, host: str, queue_name: str, port: int = 5672, prefetch_count: int = 10):
        ...
        # Simplified connection management - back to basic approach
        self.connection = None
        self.channel = None
        
        logger.info(f"Inicializando RabbitMQ Queue Middleware (simplificado): {host}:{port}/{queue_name}")
    ...
    def _ensure_channel(self):
        """Asegura que el canal esté disponible."""
    ...
    def _ensure_connection(self):
        """Ensures a connection and channel are available."""
    ...
    def start_consuming(self, on_message_callback: Callable):
        ...
        Implementa Consumer Acknowledgements con gestión robusta de conexiones.
        ...
        for attempt in range(max_retries):
            try:
                ...
                with connection.get_channel() as channel:
                    ...
                    # Configurar QoS para control de flujo optimizado
                    ...
                    self.consuming = True
                    logger.info(f"Iniciando consumo robusto de la cola '{self.queue_name}' con ACK manual")
                    
                    # Comenzar a consumir mensajes
                    try:
                        while not self._shutdown_requested:
                            connection.get_connection().process_data_events(time_limit=1)
                        ...
                        if self._consumer_tag:
                            try:
                                ...
                                logger.warning(f"Error cancelando consumidor: {e}")
                
                # If we reach here, consumption ended normally
                break
                
            except (pika.exceptions.AMQPConnectionError, ConnectionError) as e:
                logger.warning(f"Error de conexión en intento {attempt + 1}: {e}")
                self._return_connection()  # Return faulty connection
                
                if attempt < max_retries - 1:
                    time.sleep(retry_delay * (2 ** attempt))  # Exponential backoff
                else:
                    raise MessageMiddlewareDisconnectedError(f"Pérdida de conexión con RabbitMQ después de {max_retries} intentos: {e}")
    ...
    def send(self, message):
        """
        Envía un mensaje con Publisher Confirms y gestión robusta de conexiones.
        ...
        for attempt in range(max_retries):
            try:
                ...
                with connection.get_channel() as channel:
                    ...
                    # Enviar el mensaje - las confirmaciones son manejadas por el context manager
                    channel.basic_publish(
                        ...
                        mandatory=False  # Less strict routing
                    )
                    
                    logger.debug(f"Mensaje enviado exitosamente a la cola '{self.queue_name}'")
                    return
                ...
                logger.warning(f"Error de conexión en envío, intento {attempt + 1}: {e}")
                self._return_connection()  # Return faulty connection
    ...
    def stop_consuming(self):
        ...
        self.consuming = False
        logger.info(f"Solicitado detener consumo de la cola '{self.queue_name}'")
...
class RabbitMQMiddlewareExchange(MessageMiddleware):
    ...
    def __init__(
        ...
        self._shutdown_requested = False
        self._consumer_tag = None
        self._connection_pool = get_connection_pool(host, port)
        self._current_connection: Optional[RobustRabbitMQConnection] = None
        self.channel = None
        self.connection = None
        
        logger.info(f"Inicializando RabbitMQ Exchange Middleware con gestión robusta: {host}:{port}/{exchange_name} ({exchange_type})")
    
    def _get_connection(self) -> RobustRabbitMQConnection:
        """Get a robust connection from the pool."""
        if not self._current_connection or self._current_connection._is_closed:
            self._current_connection = self._connection_pool.get_connection()
        return self._current_connection
    
    def _return_connection(self):
        """Return connection to pool."""
        if self._current_connection:
            self._connection_pool.return_connection(self._current_connection)
